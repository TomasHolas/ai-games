...
export const MatchView: React.FC<MatchViewProps> = ({
    ...
    if (isReviewing && currentLogItem) {
        displayGameState = {
            ...
            is_thinking: false,
            game_over: replayStep === logs.length - 1 && (logs[logs.length - 1]?.move?.includes("Winner") || logs[logs.length - 1]?.move?.includes("Draw") || logs[logs.length - 1]?.winner_index !== null),
            winner: currentLogItem.winner || null,
            winner_index: currentLogItem.winner_index !== undefined ? currentLogItem.winner_index : null,
